...
Estas oscilações são ornamentos muito apreciados e possuem nomes
específicos. Em linguagem pianística,
os tremolos são ocorrências em que se alternam rapidamente 2 notas (com
e casos de expansão), ou seja, seriam mais facilmente associados aos vibratos
segundo a nomenclatura que estamos usando\footnote{Nossa nomenclatura tem por base
a teoria musical tradicional, que abarca nomenclaturas de diversos intrumentos e
tradições, de forma que não conflui com algumas definições de domínios mais específicos.}.
...
como eventos identificáveis. Neste caso, as oscilações se apresentam como espectro
...
Para nosso caso, em resumo, podemos entender a síntese FM através do caso entre senóides
...
\begin{equation}
\begin{split}
\{t_i'\}_0^{\Lambda -1} & = \left \{ \cos(f . 2 \pi \frac{i}{\Lambda-1} + \nu . sen( f' . 2 \pi \frac{i}{\Lambda-1} )  ) \right \}_0^{\Lambda-1}  = \\
 & = \left \{ \sum_{k=-\infty}^{+\infty} J_k(\nu) \cos \left ( f . 2 \pi \frac{i}{\Lambda-1} + k . f' . 2 \pi \frac{i}{\Lambda-1} \right )  \right \}_0^{\Lambda-1} = \\
 & = \left \{ \sum_{k=-\infty}^{+\infty} J_k(\nu) \cos \left ( (f+k.f') . 2 \pi \frac{i}{\Lambda-1} \right )  \right \}_0^{\Lambda-1}
\end{split}
\end{equation}
...
filtro parametrizado auto com fmin fmax min e erro admitido (gera fir)
som de radio com filtragem q tira o que nao estiver entre 700Hz e 3kHz
chorus c a soma de varios do mesmo com pequenas variacoes de freq, amp
e flitragens.
