...
Estes sons são constituintes importantes dos sons musicais de altura definida,
...
Outra possibilidade é a geração de ruído através do espectro desejadoi, a partir
...
A figura~\ref{fig:ruidos} dipoe lado a lado o perfil espectral e
...
igualmente por todas as frequências. Podse realizar
...
o ruído rosa, basta que modificar detalhes, em especial a equação que define $\alpha_i$.
...
\item O ruído cinza e definido como
...
pode-se observar a contribuição dos harmônicos agudos e
...
voz, com intensidades e formas de onda diferentes. Seu código está o Apêndice~\ref{ap:rugosi} e disponível online como parte da \massa.
...
tímbre (dado pela forma de onda e filtragens),
...
Uma primeira possibilidade interessante é a utilização de vínculos
entre os parâmetros do tremolo e do vibrato e algum parâmetro da nota básica,
como a frequência. Assim, pode-se estabelecer
a frequência do vibrato  é diretamente proporcional à altura à qual a profundidade do tremolo é inversamente proporcional.
...
de oscilação do vibrato e do tremolo utilizadas na equação~\ref{vbrGamma}. Já $\nu$ e $V_{dB}$ são as profundidades
do vibrato e do tremolo, respectivamente. As funções $func_a$,
$func_b$ e $func_c$ são arbitrárias e dependentes das intensões musicais. A montagem 
\emph{Tremolos, vibratos e a frequência} explora
recursos como este e variações da forma de onda da oscilação com vínculos, de modo a formar um \emph{idioma musical}\footnote{Veja na próxima sessão.}. Seu código está no Apêndice~\ref{ap:tremolos} e também disponivel online como parte da caixa de ferramentas \massa.
...
Com $\Lambda_X=\lfloor \Delta . f_a \rfloor\;\;\forall\;\; X \; \in (A,D,R,\;)$ e $\xi$ um valor pequeno que torne o \emph{fade in} e o \emph{fade out} satisfatórios, e.g. $\xi=10^{\frac{-80}{20}}=10^{-4}\;$ ou $\;\xi=10^{\frac{-40}{20}}=10^{-2}$. Quanto menor for $\xi$ mais lento é o face, a exemplo de $\alpha$ da figura~\ref{fig:transicao} .
...
e $S_j$ uma estrutura musical. Esta seção é didicada às técnicas
que torna $S_j$ interessante e agradável na audição.

Os elementos de $S_j$ podem ser sobrepostos por mixagem, como na equação~\ref{eq:mixagem} e figura~\ref{fig:mixagem}, formando intervalos e acordes. Resultam no 'pensamento vertical' da música. A concatenação de $S_j$, como na equação~\ref{eq:concatenacao} e na figura~\ref{fig:concatenacao}, forma sequências melódicas e ritmos, associados ao 'pensamento horizontal' na música. A frequência fundamental $f$ e o momento de início (ataque) são as características mais importantes dos elementos de $S_j$, possibilitando, grosso modo, a preseça de músicas de alturas (harmonia e melodia) e a preseça da métrica temporal e ritmos, respectivamente.
...
além de incindir em usos cerimoniais/religiosos e étnicos.\cite{Wisnick} Os intervalos entre quaisquer dois graus da escala de 12 notas. Notados em proporções de $\varepsilon$ entre as notas (i.e. um semitom), estes intervalos são, junto a suas notações abreviadas usuais:
...
Note que se $S_i$ é uma sequência de alturas $s_i$ e mapeamos uma sequência em $\varepsilon_1$ para
$\varepsilon_2$,
resultamos em uma nova sequência $S_i'$ com $s_i'=s_i \frac{\varepsilon_1}{\varepsilon_2}$. A montagem musical \emph{Micro tom} explora recursos microtonais e seu código está no Apêndice~\ref{ap:micro}, assim como parte da \massa\ online.
...
Pode-se anotar $\hat{1},\hat{3},\hat{5}$ para o acorde do primeiro grau, formado pelo primeiro grau da escala e central para uma música tonal. Secundários, o acorde do quinto grau $\hat{5},\hat{7},\hat{2}$ ($\hat{7}$ sustenido no caso da escala menor) e o acorde do quarto grau $\hat{4},\hat{6},\hat{1}$. Depois são considerados os outros graus. As convenções e técnicas apresentadas ao longo da história é o objeto de estudo da harmonia tradicional.\cite{Harmonia}

A chamada harmonia funcional atribui funções a estes três acordes centrais e busca compreender seus usos através destas funções. O acorde formado sobre o primeiro grau é o acorde de tônica e tem a função de manter um centro, um "chão" na música. O acorde formado sobre o quinto grau é o acorde de dominante e tem a função de tender à tônica, direcionar a música para ela. A tríade formada sobre o quarto grau é a subdominante e tem a função de se distanciar da tônica. O sistema se baseia em afirmar a tônica através de encadeamentos tônica-dominante-tônica expandidos com outros acordes das formas mais diversas.
...
A modulação é a troca de tonalidade e se dão principalmente através de: uma dominante individual que afirma a mudança da tônica; alterações cromáticas para um acorde da nova tonalidade; e enharmonia que é a mudança de função de um acorde com as mesmas notas. A importância da dominante torna ela o pivô natural das modulações, resultando no circulo das quintas.\cite{Harmonia,Schachter,Koellheuteur,Harmony}
...
Na condução de diferentes vozes, os movimentos a notas vizinhas soam mais coerentes. A importância melódica recai sobre as vozes mais aguda e mais grave.
...
de forma que soem independentes. Crucial para isso
...
é um tipo de movimento oblíquo.
A regra de ouro é cuidar que os movimentos diretos
não resulte em consonância perfeita. O movimento paralelo
...
e precedidas de consonâncias em graus conjuntos.\cite{Fux,Tragtenberg,SchoenbergContra}
...
a transição da
de durações ouvidas alturas para a apreciação em ritmo da seguinte forma:\cite{Alfaix, microsound}
...
A banda de durações marcada como transição está minimizada pois os limites não são bem definidos: a duração em que começa-se a perceber uma frequência fundamental ou uma separação entre as ocorrências é dependente da pessoa e de características do som.\cite{microsound,Roederer}
...
dada sequência musical que tenha métrica resulta
assim especificada:
...
A montagem \emph{Poli Hit Mia} utiliza as diferentes métricas e está no Apêndice~\ref{ap:poli} e disponível online como parte da \massa.
...
Na subseção seguinte exploramos arcos cíclicos e neste
...
outra que, de forma paradigmática, volta do ápice à região de partida. Canonicamente, distingue-se entre arcos cujos clímax estão: no começo, no meio, no final, na primeira metade e na segunda metade como esquematicamente a dispostos na figura~\ref{fig:climax}.\cite{Schoenberg}
...
Na música, as permutações estão são ubíquas
e estão presentes em técnicas.
A aplicação sucessivas das permutações gera arcos cíclicos.\cite{change,Zamacois,permMusic}
Esta abordagem resultou em dois trabalhos acadêmicos para geração de estruturas musicais.\cite{figgusOriginal, figgusEspacializacao}
...
Todo elemento $p$ operado consigo mesmo um número suficiente de vezes resulta no elemento neutro $p^n=e$, pois caso contrário a propriedade do fechamento resultaria em um grupo infinito gerado por $p$. O menor $n\;:\;p^n=e$ é chamado de ordem do elemento. Assim, uma permutação finita $p$ aplicada sucessivamente resulta na ordenação inicial dos elementos, formando um ciclo. Um arco musical se consultados os elementos em cada estágio.

Estes arcos podem ser efetuados pelo uso conjunto de diferentes permutações. Como exemplo
histórico, a tradição chamada \emph{change ringing} concebe música através de sinos tocados um após o outro e então tocados novamente, mas em uma ordem diferente. Este processo se repete até que se atinja a ordenação inicial. O conjunto de ordenações diferentes percorridas é um \emph{peal}. A tabela~\ref{tab:change} dispoe um \emph{peal} tradicional de 3 sinos (1, 2 e 3) que explora todas as suas ordenações. Cada linha apresenta uma ordenação dos sinos a ser tocada. As permutações estão entre as linhas.
...
Se $i$ é vai de $0$ a $n$, e $n>4$, a cada compasso
...
claros e quantitativos (sessão~\ref{notaBasica}). Em seguida, a nota foi adentrada e foram compreendidas suas transições internas e de tratamentos imediatos (sessão~\ref{varInternas}). Por fim, esta última sessão dedica-se a organizar estas notas musicais em música.
