...
\section{Variações na nota musical básica}

Nossa nota musical digital básica está bem definida com os parâmetros:
duração, altura, intensidade (volume) e timbre. Esta é uma definição
...
síntese de sons com interesses musicais pois é como eles
se apresentam na natureza. 
...
e a complexidade da nossa cognição. Mesmo assim, podemos apontar
...
período de onda possa ser usado para sintetizar sons em toda a gama
audível de frequências.

Descrevemos esta busca. Seja $\Lambda_{LUT}$ o tamanho 
da tabela e $L_i^{LUT}$ a tabela de elementos $l_i$ de um
período de onda qualquer (veja equação ~\ref{periodoUnico}). Um som
de duração $T_i^{f,\Delta}$ de frequência $f$ e duração $\Delta$
pode ser feito a partir de $L_i^{LUT}$ da seguinte forma:

\begin{equation}
T_i^{f,\Delta}=\{t_i^f\}_0^{f_a . \Delta} = \{ l_{\lfloor i . \gamma \rfloor  \% \Lambda} \}_0^{f_a . \Delta} , \gamma = \frac{f . \Lambda}{f_a}
\end{equation}
