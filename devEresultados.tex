...
A figura ~\ref{fig:amostras6} mostra uma decomposição senoidal para o caso de 6 amostras. A figura ~\ref{fig:formas6} decompõe as formas de onda básicas para o caso de 6 amostras.
...
pois este comumente é emitido por uma fonte localizada, como ocorre com um instrumento musical ou uma pessoa. Além disso, a espacialização é atributo bastante valorizado
 especialmente por audiófilos e pela indústria fonográfica~\cite{floEsp}.
Segundo a compreensão atual do fenômeno de percepção da localização espacial do som, nosso sistema nervoso utiliza
três informações: o atraso de chegada do som entre um ouvido e o outro, a diferença de intensidade do som em cada ouvido e, por último mas também crucial, a 
...
Convertendo para amplitude temos $DII_a=\frac{d'}{d}$. A $DDI_a$ pode
ser utilizada para multipicação da sequência e realizado um sinal estéreo junto à sequência original.

Embora simples até aqui, a localização espacial depende drasticamente de outras pistas. De forma canonica, resolvemos pelas
...
do cone. Nestes pontos, a inferência do ângulo azimutal depende especialmente da filtragem atenuante nos agudos, pois a cabeça interfere um tanto mais nas ondas mecânicas agudas do que das graves~\cite{Heeger,hrtf}. 

A figura~\ref{fig:spac} mostra o cone de confusão e esta sombra do crânio, além da convenção usual da especificação do ângulo azimutal positivo para a direita.

Já a localização completa, incluindo distância e elevação da fonte sonora, é dada pela função de transferência de cabeça (HRTF - do inglês \emph{Head Related Transfer Function}, bastante pessoal\ref{hrtf}. Exitem bases abertas e conhecidas de HRTF como \emph{CIPIC}~\cite{CIPIC} e podemos aplicar estas funções de transferência em um som por convolução~\ref{eq:conv}.
...
Onde $V_{dB}$ é profundidade da oscilação em decibels do tremolo e $a_{\text{máx}}=10^{\frac{V_{dB}}{20}}$
...
\begin{figure}[h!]
    \centering
    \caption{Tremolo de profundidade $12dB$ com padrão oscilatório de uma dente de serra $1,5Hz$ em uma senoide de $40Hz$}
...
estabelecida pela convolução com impulsos na sequência musical \emph{trezinho de caipiras impulsivos}.
