...
do ciclo $\lambda_{f_0}=f_a . \delta_{f_0} =f_a/f_0$. Resulta evidente que, sendo $T_i^f$ a sequência sonora de frequência fundamental $f=f_0$:
...
As formas de onda especificadas em ~\ref{denteDeSerra}, ~\ref{triangular} e ~\ref{quadrada} são dispostas graficamente em ~\ref{fig:formasDeOnda}. 
São as formas de onda artificiais tradicionalmente utilizadas na música para síntese e controle oscilatório de variáveis e apresentam diversos usos fora da música.
...
Estas formas de onda possuem usos especiais na música e seus espectros estão dispostos na figura ~\ref{fig:espectroDeOndas}. É importante notar as componentes isoladas e exatamente harmônicas dos espectros. A senóide consiste de um nódulo único no espectro, frequência pura. A dente de serra é a únida com a série harmônica completa (pares e ímpares). Já as ondas triangular e quadrada possuem as mesmas componentes espectrais, mas com decaimentos de $-12dB/oitava$ e $-6dB/oitava$.
...
O espectro harmônico é formado pelas frequências múltiplas da frequência fundamental $f_n=n.f_0$. Como nossa percepção segue a escala logarítmica, o espectro possui notas diferentes da frequência fundamental. Além disso, o número de harmônicos será limitado pela frequência máxima $t_a/2$. 

Musicalmente crucial aqui é internalizar que a presença de uma componente em uma frequência $f_n$ na decomposição por fourier possui alguma energia
   \footnote{A energia total equivale à soma dos quadrados das amplitudes como as de pressao/voltagem pelo tempo,e.g. ~\ref{fig:formasDeOnda}.
...
se e somente se existe uma contribuição senoidal, puramente harmônica no som, naquela frequência $f_n$. Esta energia concentrada especificamente na frequência $f_n$ é separada
 pelo ouvido para adentrar em um nível cognitivo de processamento\footnote{Esta separação em frequência é realizada por diversas espécies através de mecanismos similares à cóclea humana.}.
  As componentes senoidáis são geralmente as principais responsáveis pela qualidade chamada timbre e, caso não apresentem proporções harmônicas (relações de pequenos números), são ruídos, i.e. não notas com frequência fundamental como o lá 4 ($440 Hz$) de nossos exemplos. Além disso, nossa noção de altura em um complexo sonoro é baseada na semelhança do espectro com a série harmônica.

No caso de uma forma de onda fixa (e de tamanho fixo), o espectro é sempre harmônico e estático. Fixada a frequência (inverso do comprimento da onda), cada forma de onda é composta de proporções específicas das componentes harmônicas e 
...
seu espectro é perfeitamente harmônico, sem os ruídos e variações típicas do fenômeno natural. Isso pode ser visto claramente na figura ~\ref{fig:espectroOboe} onde a nota original do oboé e uma nota 
...
Além deste papel-chave no timbre e cognição, a presença e comportamento destas componentes é peculiar no som discretizado. Considere um sinal $T_i$ e sua decomposição de Fourier $\mathcal{F}\{T_i\}=C_i=\{c_i\}_0^{\Lambda-1}$. Sabemos que a recomposição segue a conversão\footnote{Lembrando que o fator $\frac{1}{\Lambda}$ pode ser distribuído dentre a transformada e a reconstrução como preferir}:
...
Onde $c_k = a_k + j . b_k$ e $w_k=\frac{2\pi k}{\Lambda}$. Nosso som possui amostras $t_i$ reais, e sabemos a contribuição de cada frequênciindexada com $k$ e de valor $c_k$ pelos termos reais da equação ~\ref{recomposicaoFourier} que resultam em senóides simples, cada uma com uma amplitude e fase:
...
\begin{figure}[h!]
    ...
    \caption{Oscilação de 2 amostras (frequência máxima em qualquer $t_a$)}
        \includegraphics[width=\textwidth]{figuras/amostras2c}
        \label{fig:amostras2}
\end{figure}
...
A figura ~\ref{fig:amostras2} exibe duas amostras e a componente espectral que contém. A decomposição de Fourier resulta neste caso em somente dois coeficientes $\{c_k=a_k-j.b_k\}_0^{\Lambda-1=1}$. O papel das amplitudes $a_k$ fica bem claro:
 $a_0$ é controla o deslocamento de fixo\footnote{chamado de bias ou offset} e $a_1$ especifica a amplitude da oscilação em si. A frequência que possui a energia\footnote{Lembrando que as energias garantidamente se equivalem pela $\frac{1}{\Lambda} . \sum_{k=0}^{\Lambda -1}c_k^2 = \sum_{i=0}^{\Lambda-1}t_i^2$ (Teorema de Parseval)}
 $e_k=\frac{(c_k)^2}{\Lambda=2}$ é dada pela relação $f_k=k . \frac{t_a}{\Lambda=2} $.  Ou seja, no caso de duas amostras só temos energia nas frequências $f_0=0$ e $f_1=\frac{t_a}{\Lambda=2}=t_{\text{máx}}$.

Este caso é de especial importância pois o mínimo necessário para representar uma oscilação são 2 amostras, e disso resulta a frequência de Nyquist $f_{\text{máx}}=\frac{t_a}{2}$. Esta é a frequência máxima presente em um som amostrado com $t_a$ amostras por segundo\footnote{Qualquer sinal amostrado possui esta característica, não somente o som digitalizado.}.

Vejamos o que acontece no caso de 3 amostras. Todas as sequências fixas $T_i$ de apenas 3 amostras também apresentam
somente 1 frequência, pois sua primeira harmônica usaria 1,5 amostras e ultrapassa o limite inferior de 2 amostras mínimas (a frequência da harmônica ultrapassaria a de Nyquist pois:  $\; \frac{t_a}{3}.2 > \frac{t_a}{2} $). Se $\Lambda=3$, 
os coeficiêntes $\{c_k\}_0^{\Lambda-1=2}$ da decomposição por Fourier apresentam-se em 
3 componentes frequenciais. 

Partimos de $\Lambda$ amostras reais $t_i$ e temos $\Lambda$ coeficientes complexos $c_k=a_k+j.b_k$. Os coeficientes $c_k$ se equivalem dois a dois\footnote{Módulos iguais e fases com sinais opostos; $a_{k1}=a_{k2}$ e $b_{k1}=-b_{k2}$.} pois correspondem às frequências $f_k = k.\frac{t_a}{\Lambda}, \; k \in \{0...\Lambda-1\} $
 e quando $ f_k > f_{\text{máx}} = \frac{t_a}{2} $ (o que já exclui $f_0=0$ e $f_k=\frac{t_a}{2}$), a frequência $f_k$ é espelhada para $f_l=\frac{t_a}{2} - (f_k-\frac{t_a}{2})=t_a-f_k=t_a - k\frac{t_a}{\Lambda}=(\Lambda-k)\frac{t_a}{\Lambda} \Rightarrow f_k=f_{\Lambda-k}$. O mesmo pode ser observado com $w_k=f_k.\frac{2\pi}{t_a}$ e $w_{\text{máx}}=\pi$ resultando em $w_k=w_{\Lambda-k}$.
  ...
  Dito de outra forma, em uma decomposição de $\Lambda$ amostras, as $\Lambda$ componentes frequenciais $\{c_i\}_0^{\Lambda-1}$ resultantes
   ...
   Excepcional excessão para $f_0$ e, no caso de $\Lambda$ ser par, de $f_{\Lambda/2}=f_{\text{máx}}=\frac{t_a}{2}$. Além disso, estas duas frequências (a frequência zero e a frequência máxima) não são representadas com variação de fase e, portanto, são estritamente reais. Assim, podemos 
...
f_{k}=f_{\Lambda-k}\;, \;\; w_{k}=w_{\Lambda-k}\;\;\;, \quad \;\; \forall \quad 1 \leq k \leq \tau  
\end{equation}
...
\begin{multline}
w_k=-w_{\Lambda-k}, a_k=a_{\Lambda-k},  b_k=-b_{\Lambda-k} \Rightarrow   \\
\Rightarrow \frac{1}{\Lambda}\sum_{k=0}^{\Lambda-1}\sqrt{a_k^2 + b_k^2} \; cos\left[w_k i - tg^{-1}\left(\frac{b_k}{a_k}\right)\right]
\end{multline}

Com 4 amostras, podemos representar 1 ou 2 frequências em proporções diferentes. A figura ~\ref{fig:amostras4} mostra estas frequências sendo realizadas compondo as formas de onda básicas e se remontando.

A figura ~\ref{fig:amostras8} mostra as decomposições senoidais para o caso de 8 amostras.


\begin{figure}[h!]
    \centering
    \caption{Frequência única em quaisquer 3 amostras}
        \includegraphics[width=\textwidth]{figuras/amostras3}
        \label{fig:amostras3}
\end{figure}
