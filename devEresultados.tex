...
Dadas as estruturas básicas musicais frequenciais (acordes e escalas) e rítmicas (divisões e aglomerações simples, compostas e complexas) é natural pensar em como manifestar estas estruturas de forma musical.

Neste ponto, encontramos alguns artifícios consagrados pelo tempo e pelo uso. Apresentaremos aqui uma possibilidade musical dada por estruturas direcionais. Na parte seguinte exploraremos as estruturas recorrentes e cíclicas.
...
uma que atinge um ápice (chamado literalmente de clímax) e 
outra que volta do ápice à nota em que se estava antes.

Seja $S_i=\{s_i\}_0^{\Lambda_S-1}$ uma sequência convergente. A sequência 
$R_i=\{r_i\}_0^{2\Lambda_S -2}=\{s_{(\Lambda_S-1-|\Lambda_S-1-i|)}\}_0^{2\Lambda_S-2}$ é uma sequência perfeitamente simétrica. Pensada musicalmente, o clímax está exatamente no meio da sequência. Canonicamente, distinguimos também entre arcos cujos clímax estão deslocados do meio para o final e para o começo (e casos extremos disso, em que os climax estão no extato começo ou final).
