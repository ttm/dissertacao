...
\begin{figure}[h!]
    ...
    \caption{3 amostras apresentam fixas apresentam uma só frequência não nula. $c_1=c_2^*$ e $w_1 \equiv w_2$}
        \label{fig:amostras3}
\end{figure}
...
pois este comumente é emitido por uma fonte localizada, como um instrumento musical ou uma pessoa. Além disso, a espacialização é atributo bastante valorizado
...
da onda vibratória\footnote{A mudança do volume (qualidade psicofísica) ocorre por diversas características 
do som como a reverberação e a concentração de harmônicos agudos, e não apenas pela energia da onda.
Para nós, entretanto, a mais facilmente controlada é de fato a energia da onda (veja equação ~\ref{potencia}) e esta também pode variar de diferentes formas.
Uma forma mais simples e conveniente é variar a amplitude pela multiplicação da sequência toda
por algum número real. A variação de energia sem variação de
amplitude é comumente chamada de \emph{compressão sonora} e é muito útil e corriqueira na
produção musical atual~\cite{compressao}.}. Podemos obter este resultado multiplicando a sequência original
...
\begin{equation}
T_i^{'}=\left \{ t_i \left(10^{\frac{V_{dB}}{20}} \right)^{\left ( \frac{i}{\Lambda-1} \right )^\alpha} \right \}_0^{\Lambda-1}
\end{equation}
...
\subsection{Usos musicais parte 1: tremolo e vibrato, AM e FM}
