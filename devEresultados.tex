...
As sequências podem também ser concatenadas no tempo. Caso as sequências $\{t_{k,i}\}_0^{\Lambda_k-1}$ de tamanhos $\Lambda_k$  representem notas musicais, sua concatenação em $t_i$ resultará em sequências musicais simples ou melodias:
...
e a complexidade da nossa cognição sonora. Mesmo assim, podemos apontar
os recursos primários para estas variações dos parâmetros da nota
básica.
...
\begin{equation}
T_i^{f,\Delta}=\{t_i^f\}_0^{f_a . \Delta-1} = \{ \widetilde{l}_{\lfloor i . \gamma \rfloor  \% \widetilde{\Lambda} } \}_0^{f_a . \Delta-1}\; , \quad \text{onde} \;\; \gamma = \frac{f . \widetilde{\Lambda}}{f_a}
\end{equation}

Ou seja, dada a tabela de busca, basta termos a sequência de índices corretos
para sintetizar o som. No termo $i . \gamma$ não importa se tomamos o inteiro
como a parte inteira da multiplicação, como o maior inteiro acima da multiplicação
ou se arredondamos. Além disso, para fins de síntese em $44,1 kHz$,
 o padrão é usar $\widetilde{\Lambda} = 1024$ amostras, pois já não gera ruído
 relevante no espectro audível.

 A expressão que define a variável $\gamma$ pode ser facilmente compreendida da
 seguinte forma: a variável $i$ é acrescida de $f_a$ em $1$ segundo (pela
 definição de $f_a$ e $i$, pois temos $f_a$ amostras $i$ em $1$ segundo). Caso a dividamos por $f_a$, teremos $\frac{i}{f_a}$,
fração esta que é acrescida de $1$ a cada $1$ segundo. Multiplicado pelo comprimento da
 tabela $\widetilde{\Lambda}$, teremos $\frac{i . \widetilde{\Lambda}}{f_a}$
 que resulta na varredura completa da tabela $\widetilde{L_i}$ em 
 1 segundo com o acrescimo de $\widetilde{\Lambda}$ em $1$ segundo. Por fim,
 se multiplicarmos pela frequência $f$ que queremos, teremos $\frac{i . f . \widetilde{\Lambda}}{f_a}$
 que resulta em $f$ varreduras completas da tabela $\widetilde{L_i}$ em $1$ segundo, i.e. a sequência
 resultante apresenta frequência $f$.

Importantes considerações: $f$ é qualquer, só há limitantes nas frequências
graves quando $\widetilde{\Lambda}$ não é suficientemente grande para a taxa de amostragem
$f_a$\footnote{$\widetilde{\Lambda} = 1024$ é suficiente para sintetizar
sequências $T_i^f$ com qualquer $f$ no espectro.}. O procedimento de busca em tabela
é bastante barato, substituindo cálculos por buscas simples (por isso geralmente
é entendido como um processo de otimização). Salvo quando assinalado,
usaremos este procedimento para todos os casos cabíveis a seguir pois
simplifica diversas rotinas e é a forma usual de realizar estas sínteses.
