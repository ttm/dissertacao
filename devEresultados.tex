...
Musicalmente crucial aqui é internalizar que se um sinal real foi decomposto por fourier e possui uma componente em uma frequência $fn$ com alguma energia
...
então existe uma contribuição senoidal, puramente harmônica no som. Esta energia concentrada especificamente na frequência $f_n$ é separado pelo ouvido\footnote{A cognição cerebral se dá basicamente com estas componentes. Muita informação é obtida pelo processamento destes sinais acústicos por diversas diversas formas de vida e se baseia em modulações de frequência e amplitude.}
