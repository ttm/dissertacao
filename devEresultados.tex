...
por ao menos $50-63ms$. Para que a separação entre eles possa ser apreciada, ela deve ser um pouco maior,
por volta de $100ms$ ($10Hz$)(\ref{microsound}). Podemos sumarizar a transição do domínio da apreciação
...
\hline
-           & \multicolumn{10}{c}{$\underleftarrow{\text{\bf zona de percepção temporal em rítmo}}$} & \multicolumn{2}{c}{transição} & \multicolumn{3}{c}{-} \\
...
-           & \multicolumn{10}{c}{ - } & \multicolumn{2}{c}{transição} & \multicolumn{6}{c}{$\overrightarrow{\text{\bf zona de percepção temporal em altura}}$} \\
\hline
\label{altRit}
%\ca
...
e são dependentes da pessoa e de características do som como o conteúdo espectral e intensidade.

A métrica rítmica costuma se basear em uma duração básica chamada pulso. O pulso tipicamente
compreende durações entre $0,25-1,5s$ ($40-240BPM$) que se costuma associar às durações
entre batidas do coração ou entre os passos ao caminhar.
...
sequenciais no tempo ocorrem com fatores 2, 3, depois seus múltiplos
4 (2 grupos de 2) e 6 (dois grupos de 3 ou 3 grupos de 2)
e então 8 (2 grupos de 4), 9 (3 grupos de 3) e por último os primos 5 e 7, completando 1-9 (~\ref{jocelyn}, ~\ref{favre}).}.

As relações duais costumam a ocorrer em rítmos de dança
e ocasiões festivas e são chamadas imperfeitas. As relações triádicas ocorrem
mais na música ritualística e relacionada ao sagrado
e são ditas perfeitas.

Seja $P_i^j$ a $i$-ésimo unidade de pulsos no nivel $j$ de agrupamento. Assim,
$P^1_{10}$ é o décimo pulso, $P^{2}_3$ é a terceira unidade de agrupamento de pulsos,
$P^{-1}_2$ é a segunda unidade da subdivisão do pulso. Especial atenção para
...
Sempre que $i$ soma três índices temos uma relação \emph{perfecta}, quando soma dois
ou quatro índices temos uma relação \emph{imperfecta}.


Podemos convencionar $P^{-1,1,3}_{3,2,2}$ como a terceira subdivisão $P^{-1}_3$ do segundo
pulso $P^1_2$ do terceiro aglomerado $P^3_2$ de aglomerado de pulsos ($P^2_1$ omitido por simplicidade).
Tipicamente, para um dado $j$, $i$ tende a se manter.

A cada unidade ou conjunto de unidades $P_i^j$ pode ser associada uma sequência de amostras temporais $T_i$
que formam uma nota musical. A associação de $P^{-1,1}_{ i_{\text{fraco}},x }$ e $P^{-1,1}_{0,x+1}$ é o que
se chama de síncopa.

Um compasso 4 por 4 possui $P^1_i$ com $i$ tipicamente contando 2, 4 ou 8 subdivisões. E
em $P^2_i$, $i$ conta 4 unidades.
