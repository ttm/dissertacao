...
Os elementos de $S_j$ podem ser mixados (veja equação~\ref{eq:mixagem} e a figura~\ref{fig:mixagem}) formando acordes ou concatenados (veja equação~\ref{eq:concatenacao} e a figura~\ref{fig:concatenacao}) formando sequências melódicas.
...
Esta precisão absoluta é possível somente em implementações
computacionais, pois os intrumentos reais possuem desvios destas notas para melhor harmonia entre os harmônicos~\cite{Roederer}.
...
as escalas como sequências de fatores aos quais $\varepsilon$ é elevado, resultando
