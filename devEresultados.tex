...
Os elementos de $S_j$ podem ser mixados (veja equação~\ref{eq:mixagem} e a figura~\ref{fig:mixagem}) formando intervalos e acordes ou concatenados (veja equação~\ref{eq:concatenacao} e a figura~\ref{fig:concatenacao}) formando sequências melódicas.

\subsubsection{Afinação, escalas, acordes e intervalos}
...
além de usos cerimoniais/religiosos e étnicos~\cite{Wisnick}. Os intervalos entre quaisquer duas notas da escala de 12 notas em que se subdivide a oitava possuem nomes específicos. Notados em proporções de $\varepsilon$ entre as notas (i.e. um semitom), estes intervalos são:

\begin{equation}
\begin{split}
\text{uníssono} & = 0 \\
\text{semitom ou segunda menor} & = 1 \\
\text{tom, tom inteiro ou segunda maior} & = 2 \\
\text{terça menor} & = 3 \\
\text{terça maior} & = 4 \\
\text{quarta justa} & = 5 \\
\text{quarta aumentada, quinta diminuta ou trítono} & = 6 \\
\text{quinta justa} & = 7 \\
\text{sexta menor} & = 8 \\
\text{sexta maior} & = 9 \\
\text{sétima menor} & = 10 \\
...
Nesta nomenclatura (padrão no Brasil há algumas décadas),
...
Este arcabouço resume com exatidão a teoria dos intervalos musicais~\cite{Lacerda} e nos será útil adiante. Dispomos esta nomenclatura também por completude desta exposição, não há necessidade alguma de decorar estes nomes ou apreender estes detalhes todos enquanto não há uso efetivo.
...
Basta acrescentar $10$ para ter o acorde
com sétima menor ou $11$ para resultar em sétima maior\footnote{As sétimas
menor e maior são intervalos de $10$ e $11$ semitons respectivamente.}. As inversões e posições abertas
...
As unidades mais fortes (acentuadas) são as que consistem nas cabeças dos tempos. Nas divisões binárias (2, 4 e 8 dos casos considerados aqui),
...
Assim, $P_i^j$ é a $i$-ésima unidade de 
...
em padrões de permutações dentre um fixado número de notas~\cite{change}.} e a consequente realização de dois trabalhos~\cite{figgusOriginal, figgusEspacializacao}.

Qualquer conjunto de permutações podem ser consideradas geradoras de grupos algébricos. Assim, vale citarmos as propriedades que definem um grupo $G$:

\begin{equation}
\begin{split}
\forall p_1,p_2 \in G \Rightarrow p_1 \bullet p_2 & = p_3 \in G\quad\text{(propriedade de fechamento)} \\
\forall p_1,p_2,p_3 \in G \Rightarrow (p_1\bullet p_2)\bullet p_3 & = p_1\bullet (p_2\bullet p_3)\quad\text{(propriedade da associatividade)} \\
\exists e \in G : p \bullet e & = e \bullet p \forall p \in G\quad\text{(existência do elemento neutro)} \\
\forall p \in G \exists p^{-1} : p\bullet p^{-1} & =p^{-1}\bullet p = e\quad\text{(existência do inverso)}
\end{split}
\end{equation}
...
Alguns teoremas nos são caros. Por exemplo, segue imediatamente que, em um grupo finito, todo elemento operado consigo mesmo um número de vezes resulta no elemento neutro. O menor número de vezes necessário para atingir o elemento neutro é chamado de ordem do elemento. Assim, uma permutação finita aplicada sucessivamente um número suficiente de vezes
...
O menor valor de $n$ é chamado de \emph{ordem} da permutação $p$. O elemento $e$ é o elemento neutro e é
a permutação que leva uma ordenação para ela mesma. Aqui já se pode notar o comportamento cíclico causado pela aplicação sucessiva da mesma permutação.
