...
\section{Música no Áudio Digital}
...
Em primeiro lugar, estamos tratando diretamente da representação digital do som. Isso constitui em uma
sequência de amostras igualmente espaçadas no tempo. Desta forma, definimos a
taxa de amostragem $t_a$ como o número de amostras por segundo do áudio digital.
A sequência $T_i={t_i}$ possui as amostras separadas por $\delta_a=1/t_a$ segundos.

Desta forma, visando uma nota musical de duração $\Delta$,
teremos uma sequência de $ \Delta . t_a $ amostras:

\begin{equation}
T_{i}^{\Delta}={\{t_i\}}_{0}^{\Delta . t_a - 1}
\end{equation}
...
\begin{equation}
pot(T_i^{\Delta})=\frac{\sum_{i=0}^{\Delta t_a -1} t_i^2}{\Delta t_a}
\end{equation} 

Como o volume final dependerá da amplificação do sinal nos falantes de quem ouvir, o que conta mais é a potência relativa de uma nota em relação às outras ou de um trecho da música em relação ao resto. Lembrando que, devido à nossa percepção logarítmica, a redução para uma mesma fração é sentido como a mesma diminuição do volume.
