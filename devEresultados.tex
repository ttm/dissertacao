...
Segundo a lei de Weber e Fechner, nossa percepção tem uma relação logarítmica com
o estímulo que a causa. Mesmo assim, dado o uso, explicitaremos também a variação
linear. Começaremos por ela por razões didáticas.
...
temos o deslocamento $f_0$ na imagem, $f_i$ varre o contra-domínio enquanto $i$
...
da frequência. Nossa percepção segue a progressão logarítmica, então uma progressão de frequência
\emph{percebida} como linear segue a progressão logarítmica. Fazendo a varredura com $i$
...
Para realizarmos o caso mais geral, façamos uma sequência $t_i'$
de frequência $f'$ com o auxílio
de uma segunda tabela $\widetilde{M}_i$ de tamanho $\widetilde{\Lambda}_M$ que apresente também
um período de onda que oscila entre $[-1,1]$. Descrevemos um vibrato da seguinte forma:
...
\begin{equation}
f_i=f \left ( \frac{f + \mu }{f} \right )^{t_i'}
\end{equation}

\begin{equation}
\gamma_i = \left \lfloor i . f_i \frac{\widetilde{\Lambda}}{f_a} \right \rfloor = \left \lfloor i \frac{\widetilde{\Lambda}}{f_a}f \left ( \frac{f + \mu }{f} \right )^{t_i'}  \right \rfloor
\end{equation}
...
O caso do tremolo é semelhante. Dado $f'$, $\gamma_i'$ e $t_i'$ permanecem os mesmos. Calculamos
...
\begin{equation}
T_i^{f, tr(f')}=\{ t_i^{f,tr(f')} \}_0^{\Lambda-1}=\{ t_i^{f} . a_i \}_0^{\Lambda-1}=\{t_i^f . a_{\text{máx}}^{t_i'}\}_0^{\Lambda-1}
\end{equation}

Onde $V_{dB}$ são os decibeis do tremolo e $a_{\text{máx}}$ é o ganho máximo de amplitude envolvido.
