...
The human auditory system perceives sounds within the frequency bandwidth between $20Hz$ and $20kHz$,
...
as one can perceive in the constitution of sounds of musical interest
...
from musical scores and phonetic texts to electric analog signals and binary data,
which includes arbitrary sets of features and spectral decompositions such as obtained through wavelets.
When the representation mimics the mechanical wave directly, it might be referred to as audio,
although the terms audio and sound are often used without distinction
and has many definitions that depends on the context and the author~\cite{audioSound}.
In this definition, which contemplates most common uses of the word,
...
Digital audio protocols that facilitate file
storage and transfer often implies quality loss and are called \emph{lossy}.
This is the case e.g. of MP3 and Ogg Vorbis~\cite{loss}.
Non-lossy representations of digital audio, called \emph{lossless}, on the other hand,
assures perfect reconstruction of the analog wave within any convenient precision.
The standard paradigm of lossless audio consists of
representing the sound with samples equally spaced
by a duration $\lambda_s$, and specifying each sample by a fixed number of bits.
This is the Pulse Code Modulation (PCM) representation of sound.
A PCM audio format has two essential attributes:
a sampling frequency $f_s=\frac{1}{\lambda_s}$ (also called the sampling rate),
which is the number of samples used for representing a second of sound;
and a bit depth, which is the number of bits used for representing the amplitude of each sample.
...
\begin{figure*}
	...
	\caption{Example of PCM audio: a sound wave is represented by 25 samples where each sample has an amplitude specified with 4 bits.}
        \label{fig:PCM}
\end{figure*}
...
is twice the maximum frequency of the represented signal.
Thus, for general musical purposes, it is necessary to have samples in a rate
...
to encompass concepts beyond e.g. rhythm, melody and harmony.
This is very evident e.g. in the concrete, electronic, electroacoustic,
and spectral musical styles.
In the 1990s, it became evident that popular music had also incorporated
sounds amalgams and abstract discursive arcs,
to name just a couple of simple characteristics.
...
Thereafter, in this article we specify musical notes in PCM audio through equations
...
This article aims at representing musical structures by their discrete-time sonic characteristics.
Results include mathematical relations, usually in terms of musical characteristics and samples,
...
there are some books and computer implementations that tackle the subject.
...
although current music software
use the analytical descriptions presented in this study,
it seems that there has been no concise and mathematical description of them.
...
\begin{figure*}
    ...
    \caption{Consultation in the \emph{lookup table} to synthesize sounds at different frequencies using a unique waveform with high resolution.
...
\begin{figure*}
    ...
    \caption{Convolution with the impulse: shifting (a), delay lines (b) and granular synthesis~(c). Represented in increasing order of its pulse density. The vertical axis is related to amplitude although one should keep in mind that each plot has two or three displaced signals.}
        \label{fig:delays}
\end{figure*}
...
\begin{table*}[htp!]
\caption{Transition of durations heard as rhythm until heard as pitch.}
