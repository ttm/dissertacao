% mi\tableofcontents % enable to generate .toc file used in listings.tex
...
such frequency limits correspond to  wavelengths of $\frac{343.2}{20} \approx 17.16\,m$ and
...
Digital audio protocols often imply quality loss (to yield smaller files, ease storage and transfer)
...
\begin{enumerate}
	...
	\item Provide approachability to the developed framework~\footnote{All
		...
		Part of the scripts has been transcribed to JavaScript (which favors their use in Web browsers such as Firefox and Chromium) and native Python~\cite{numpy, audiolab, tutpython, python}.
...
This duration, multiplied by the sampling frequency $f_s$, yields the number of samples per cycle: $\lambda=f_s . \delta =f_s/f$.
...
While the sinusoid is an isolated node in the spectrum, any other waveform presents a succession of harmonic components.
...
$\Lambda$ real samples $t_i$ result in $\Lambda$ complex coefficients $c_k=a_k+j.b_k$. The coefficients $c_k$ are equivalent two by two, corresponding to the same frequencies and with the same contribution to its reconstruction. They are complex conjugates: $a_{k1}=a_{k2}$ and $b_{k1}=-b_{k2}$ and, as a consequence, the moduli are equal and phases have opposite signs. Recalling that $f_k = k\frac{f_s}{\Lambda}, \; k \in \left\{0, ..., \left \lfloor \frac{\Lambda}{2} \right \rfloor \right\} $. When $k > \frac{\Lambda}{2}$, the frequency $f_k$ is mirrored through $\frac{f_s}{2}$ in this way: $f_k=\frac{f_s}{2} - (f_k-\frac{f_s}{2})=f_s-f_k=f_s - k\frac{f_s}{\Lambda}=(\Lambda-k)\frac{f_s}{\Lambda} \;\;\;\; \Rightarrow \;\;\;\; f_k\equiv f_{\Lambda-k} \; ,\;\; \forall \;\; k<\Lambda$. 
...
To discuss the general case for components combination in each sample $t_i$, one can gather relations in Equation~\ref{moduloEfase} for the real signal reconstruction, equivalences of moduli (Equation~\ref{equivalenciasModulos}) and phases (Equation~\ref{equivalenciasFases}), the number of paired coefficients (Equation~\ref{coefsPareados}), and the equivalence of paired frequencies (Equation~\ref{equivalenciasFreqs}):
...
There are reverberation models less based on each independent reflection and that explore valuable cues to the auditory system. In fact, a reverberation can be modeled with a set of two temporal and two spectral regions~\cite{JOSPhy}:
\begin{itemize}
...
In music its usage transcends these applications: it simplifies many operations and enables the use of a single wave period to synthesize sounds in the whole audible spectrum, with any waveform.
...
design and implementation of filters is a field of recognized complexity,
...
They are well behaved filters whose frequency responses are illustrated in Figure~\ref{fig:iir}.
...
\begin{figure*}
    ...
    \caption{Moduli for the frequency response (a), (b), (c) and (d) for IIR filters of Equations~\ref{eq:passa-baixas},~\ref{eq:passa-altas},~\ref{eq:passa-banda} and~\ref{eq:rejeita-banda} respectively, considering different cutoff frequencies, center frequencies and bandwidths.}
        \label{fig:iir}
\end{figure*}

\begin{enumerate}
  \item Low-pass with a simple pole and modulus of the frequency response in the upper left corner of Figure~\ref{fig:iir}. The general equation has the cutoff frequency $f_c \in (0,\frac{1}{2})$, which is the fraction of the sample frequency $f_s$ in which an attenuation of $3dB$ occurs. The coefficients $a_0$ and $b_1$ of the IIR filter are given by:
\begin{equation}\label{eq:passa-baixas}
 ...
 \end{split}
\end{equation}
  \item High-pass filter with a simple pole and modulus of the frequency responses at the upper right corner of Figure~\ref{fig:iir}.
...
Some noises with static spectrum are listed below. They are called \emph{colored noises} since they are associated with colors for many reasons.
...
The exponential $e^{j.x}$ is a way to obtain unitary modulus and random phase for the value of $c_i$. In addition, $c_{\Lambda/2}$ is always real (as discussed in Section~\ref{sec:spectrum}).
...
It is important to correctly distinguish both tables 
...
The tremolo is similar: $f'$, $\gamma_i'$ and $t_i'$ remains the same.
...
pulse. Two signals, one with 2 impulses and the other with 3 impulses, both equally spaced in the pulse duration, yield a pulse
...
Doubling the frequency is equivalent to ascending one octave\footnote{A pitch
...
In order to account for microtonality\footnote{Microtonality is the use of intervals smaller
...
\begin{itemize}
	\item Intervals are inspected first by the number of steps between notes in some scale.
		...
		A step, in this sense, is yielded by two consecutive notes in a musical scale.
	...
	\item The tritone is a dissonance in Western music because
		it characterizes the ``dominant'' chord in the tonal system (see Section~\ref{subsec:harmonia}) and represents (or yields) instability. Some cultures consider the interval a consonance, using it in melodies and chants as a stable interval.
...
This description summarizes the traditional theory of musical intervals~\cite{Lacerda}.
...
The minor scale has two additional forms, named harmonic and melodic:
...
\begin{itemize}
        ...
        \item Major or minor thirds from the root entail major or minor chord qualities.
...
\subsection{Tonal, modal and atonal music, harmonies, harmonic expansion and modulation}\label{subsec:harmonia}
...
Most frequently, atonal music is not strictly dodecafonic, but "serial", i.e. they use the same kind of techniques as stated above, based on a sequence of pitches (called the series or row), but not within a 12 tone sequence and using sequences of other parameters (e.g. duration, timbre).
...
\footnotetext{The
	definition of the terms 'quarter note' and 'time signature' are beyond the scope of this work and might be found in elementary texts about musical theory~\cite{Lacerda}.
	Anyway, what is important for the reader to grasp is that: 1) successive temporal elements of equal duration
	are both grouped and subdivided into other elements of equal duration;
	2) these groupings and subdivisions are usually made in terms of small integers and are discerned into
...
The rhythm is commonly based on a basic duration called pulse, whose typical duration ranges between $0.25$ and $1.5s$ ($240$
...
(refer to the beginning of this Section~\ref{notasMusica} if needed). Below is a list of basic techniques
for variation.
\begin{itemize}
...
2) the period of incidence (how many consults before a permutation is
applied).
...
PPEPPS is a set of musical pieces that uses permutations.~\cite{MASSA,figgusOriginal,figgusEspacializacao}.

\subsection{Serialism and post-serial techniques}\label{sec:ser}
...
\begin{itemize}
	\item Spectralism:
		consists in using the (Fourier) spectrum of a sound or the harmonic series for music composition,
		such as
...
of praxis possibilities are typical and highly relevant for artistic contexts~\cite{Harmonia,Webern}.
...
For sure, the possibilities are very wide, which is evidenced in the wide range of musical pieces and styles.
...
\label{cap:conclusao}
In this article, we hope to have effectively related musical elements to digital audio.
We aimed at achieving a concise presentation of the subject that involves many knowledge fields.
...
Moreover, this article and the open source scripts supplied should facilitate the use of the proposed framework.
The Supporting Information document holds listings of sections, equations, figures, tables, scripts and other documents.

The possibilities provided stem from both the organization of knowledge and the ability to achieve sounds which are true to the models.
...
Furthermore, standard software for synthesis and processing of sounds for musical purposes
by restrict the bit depth to 16 or 24.
...
The didactic potential of the framework is evident by noticing that:
\begin{itemize}
...
\end{itemize}
\noindent Significantly, this framework was used in a number of contexts, including courses, software implementations and making music~\cite{vimeoLM,vivace,repoDissertacao,dissertacao}.
Such detailed analytical descriptions, together with the computational implementations, have not been covered before in the literature, as far as the authors know (see the literature review in the Appendix G of~\cite{dissertacao}), where books, articles and open software are related to this framework.

The free software license and online availability of the content,
with the respective codes and sonic examples, facilitate collaborations and generation of sub-products in a co-authorship fashion,
...
which need validation and should be reported, especially with~\cite{quadrosSonoros}.\footnote{The sonic portraits were sent to a public mailing list and the fifth piece was reported by some individuals to induce a state in which noises from the own tongue, teeth and jaw of the individual echoed for some seconds~\cite{quadrosMetarec}.} Other foreseen advances
