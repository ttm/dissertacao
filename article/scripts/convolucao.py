...
p.plot(range(3,13),h[::-1]+1,'ro',label=r'retrogrado da resposta ao impulso $\{h_{i-j}\}_{j\; =\; max(i+1-\Lambda_h,0)}^i$')
...
c=n.convolve(t,h)
p.plot(c-2.5,'go', label=u'sinal convoluído ' + r'$\{\, (t*h)_{i}=\sum_{j=max(i+1-\Lambda_h,0)}^{i} t_j . h_{i-j}\;\}_{0}^{48+10-2=56}$')
