...
tempo mark -- and to place an impulse at the start of each beat, with the purpose of
establishing metric and rhythms: the
convolution with an impulse shifts the sound to where
the impulse.
For example, two impulses equally spaced builds a binary division into the
pulse. Two signals, one with 2 impulses and another with 3 impulses, both 
equally spaced in the pulse duration, yields the pulse
maintenance with a rhythmic trace in which is easy to use binary or ternary
divisions of the pulse, as found in many ethnic or traditional music styles~\cite{Gramani}. 
Absolute values of the impulses implyes in proportions among the
amplitudes of the sonic incidences.
...
As an example, consider $\Delta_1$ as the duration of the first period and $\Delta_R$ as the duration of total reverberation ($\Lambda_1=\Delta_1 f_s$, $\Lambda_R=\Delta_R
f_s$). It is possible to add a probability $p_i$ of a sound reincidencing in the
