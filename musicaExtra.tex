...
finalidades pedagógicas e sociais das práticas audiovisuais.
...
que sintetizam os ruídos branco, azul, violeta, rosa e marrom. Os
  ...
  repositório git como pode ser visto em \emph{online}\footnote{Este link é o \emph{README} do repositório \emph{git} com os códigos dos
      plugins:
...
extremamente dependentes da aplicação. A complexidade dos algoritmos
...
geralmente causado pela corrente alternada que
...
frequência, amplitude e timbre, até a confecção de uma ferramenta
...
2012 mas adiantada em 2011, os algoritmos em Python propriamente ditos
...
na síntese de timbres diversos e ricos em harmônicos e evolução temporal. Embora
...
sobrepõe no tempo todas as unidades, no \emph{fazSequencia} as unidades são todas justapostas no tempo.
...
de linhas melódicas), é feita com os recursos usuais da linguagem:
...
A classe do padrão de permutação possui também um período de aplicação da permutação, ou seja, de quantas em quantas leituras da sequência a permutação é aplicada.
...
com macros para execução de ritmos através de \emph{samples}
...
audiência possa ver o que está sendo escrito, no ritmo em que se
...
usos em código desta abordagem. Como pode-se observar acima, neste mesmo Apêndice, este trabalho, disponibilizado e concebido como tecnologias livres, tem dimensões bastante consideráveis em aspectos socialmente engajados e artísticos. Esta seção exibe os feitos que extrapolam a música por completo.
   ...
   \subsection{Tecnologias sociais: Sítios, Conteúdos e Articulação}
