...
def t(i): return (1./Lambda)*n.sum(C_k*n.e**(1j*w_k*i))
### 2.16 Recomposição real
def tR(i): return (1./Lambda)*n.sum(n.abs(C_k)*n.cos(w_k*i-n.angle(C_k)))
...
### 2.21 Combinação das componentes em cada amostra
    w_k=2*n.pi*n.arange(Lambda)/Lambda
def t(i): return (1./Lambda)*(A_k[0]+2.*n.sum(n.abs(C_k[1:tau+1])*\
