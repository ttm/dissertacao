...
s1=H(([adsr(v(f=200.*2.**(b/12.),d=.2))+adsr(v(f=200*2.**(ee/12.),d=.2)) for b,ee in zip(bb,E)   ]))


m=[0,7,7,7,2,7,7,5,3,5,1,0,7,1,0,0]
cc=contraNotaNotaSup(m)

s2=H(([adsr(v(f=200.*2.**(b/12.),d=.2))+adsr(v(f=200*2.**(ee/12.),d=.2)) for b,ee in zip(cc,m)   ]))
...
a.wavwrite(s,"contaPonto.wav",f_a) # escrita do som
