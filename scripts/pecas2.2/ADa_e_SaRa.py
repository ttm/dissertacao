...
def adsr(som,A=10.,D=20.,S=-20.,R=100.,xi=1e-2):
    a_S=10**(S/20.)
    Lambda=len(som)
    Lambda_A=int(A*f_a*0.01)
    Lambda_D=int(D*f_a*0.01)
    Lambda_R=int(R*f_a*0.01)
...
T=adsr(v(tabv=Tr_i ,d=2.,fv=35.,nu=7.0)*A(),100,100,xi=1e-1)
T2=adsr(v(tabv=Tr_i ,d=2.,fv=0.,nu=7.0)*A(),100,100,xi=1e-1)
